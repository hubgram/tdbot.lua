...
-- Vector<int> example form is like this: {[0] = v} or {v1, v2, v3, [0] = v}
...
local function getFilePersistent(persistent_file_id, file_type, cb, cmd)
  tdbot_function ({
    ...
    file_type = ("fileType" .. file_type) or nil
  }, cb or dl_cb, cmd)
end
...
local function getChats(offsetorder, offset_chat_id, limit, cb, cmd)
  ...
  tdbot_function ({
    _ = "getChats",
    offsetorder = offsetorder or 9223372036854775807,
...
-- with non-empty query (searchSecretMessages should be used instead) or without enabled message database
-- @chat_id Chat identifier to search in
-- @query Query to search for @sender_user_id If not 0, only messages sent by the specified user will be returned.
...
local function deleteMessages(chat_id, message_ids, revoke, cb, cmd)
  tdbot_function ({
    _ = "deleteMessages",
    chat_id = chat_id,
    message_ids = message_ids, -- vector
...
local function changeChatMemberStatus(chat_id, user_id, status, a, b, c, d, e, f, g, h, i, cb, cmd)
    if status and status == "Administrator" then
        s = {
            ...
            can_change_info = a or nil,
            can_delete_messages = b or nil,
            can_invite_users = c or nil,
            can_restrict_members = d or nil,
            can_pin_messages = e or nil,
            can_promote_members = f or nil,
            can_be_edited = g or nil,
            can_post_messages = h or nil,
            an_edit_messages = i or nil
        }
    elseif status and status == "Restricted" then
        s = {
            _ = "chatMemberStatusRestricted",
            is_member = a or nil,
            restricted_until_date = b or nil,
            can_send_messages = c or nil,
            can_send_other_messages = d or nil,
            can_add_web_page_previews = e or nil,
        }
    else
        s = {
            _ = "chatMemberStatus" .. (status or "Member")
        }
    end
...
local function toggleGroupEditors(group_id, anyone_can_edit, cb, cmd)
  tdbot_function ({
    ...
    group_id = getChatId(group_id)._,
    everyone_is_administrator = anyone_can_edit
  }, cb or dl_cb, cmd)
end
...
local function getChatEventLog(chat_id, query, from_event_id, limit, user_ids, message_edits ,message_deletions, message_pins, member_joins, member_leaves, member_invites, member_promotions, member_restrictions, info_changes, setting_changes, cb, cmd)
  ...
  filters = {
    ...
    message_edits = message_edits or True,
    message_deletions = message_deletions or True,
    message_pins = message_pins or True,
    member_joins = member_joins or True,
    member_leaves = member_leaves or True,
    member_invites = member_invites or True,
    member_promotions = member_promotions or True,
    member_restrictions = member_restrictions or True,
    info_changes = info_changes or True,
    setting_changes = setting_changes or True
  }
...
local function registerDevice(device_token, token, device_tokenset, cb, cmd)
  ...
  if device_tokenset then
    dToken = {_ = "deviceTokenSet", token = device_tokenset} -- tokens:vector<DeviceToken>
  end
...
local function reportChat(chat_id, reason, cb, cmd)
  tdbot_function ({
    _ = "reportChat",
    chat_id = chat_id,
    reason = "chatReportReason"..(reason or "other")
  }, cb or dl_cb, cmd)
end
...
local function optimizeStorage(size, ttl, count, immunity_delay, file_types, chat_ids, exclude_chat_ids, chat_limit, cb, cmd)
  tdbot_function ({
    ...
    file_types = file_types, -- vector<FileType>
    chat_ids = chat_ids, -- vector<int>
    exclude_chat_ids = exclude_chat_ids, -- vector<int>
...
-- @invoice The invoice @title Product title, 1-32 characters
